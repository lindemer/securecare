...
#define SCHED_MAX_EVENT_DATA_SIZE       MAX(MAX(sizeof(nrf_drv_twis_evt_t), MAX(sizeof(nrfx_spim_evt_t), sizeof(nrf_drv_gpiote_pin_t))), sizeof(fds_evt_t))
...
#define I2C_ENABLED          0xF0
#define I2C_DISABLED         0x0F
...
static void lidar_lite_evt_handler(lidar_lite_evt_t * p_event)
{
    ...
    switch(event_type)
    {
        ...
        case LIDAR_LITE_EVT_MEASUREMENT_COMPLETE:
            ...
            {
                NRF_LOG_INFO("Measured Distance: %d", p_event->data.distance_cm);
            }
